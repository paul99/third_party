    ...
    if (turboSwizzled(info->out_color_space)) {
         ...
         while (info->output_scanline < info->output_height) {
             ...
             if (jpeg_read_scanlines(info, &row, 1) != 1)
                 return false;
#if OS(ANDROID)
             // There is a bug in libjpeg_turbo that alpha value is not set to
             // 0xFF on android when do_fancy_sampling is set to false. Set
             // alpha values here util the bug is fixed in libjpeg_turbo.
             unsigned char* alpha = row + 3;
             for (int x = 0; x < (int) info->output_width; ++x, alpha += 4) {
                 *alpha = 0xFF;
             }
#endif
         }
