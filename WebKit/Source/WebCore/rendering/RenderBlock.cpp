...
static void FB_SET_ATTR(WebCore::RenderObject* o, const String& name, const String& value)
{
    if (!o->node() || !o->node()->isElementNode())
        return;
    WebCore::Element* element = static_cast<WebCore::Element*>(o->node());
    WebCore::ExceptionCode ec;
    element->setAttribute(AtomicString(name), AtomicString(value), ec);
}
#else
#define FB_SET_ATTR(o, name, value)
#endif
...
RenderBlock::RenderBlock(Node* node)
      ...
      , m_maxTextWidthAfterFontBoosting(-1)
      , m_wasOverflowHeightForBoosting(false)
#endif
{
...
#if OS(ANDROID)
bool RenderBlock::markFixedPositionedObjectsForLayout() {
    bool hasFixedPositionedObjects = false;
    if (m_positionedObjects) {
        RenderBox* r;
        Iterator end = m_positionedObjects->end();
        for (Iterator it = m_positionedObjects->begin(); it != end; ++it) {
            ...
            if (r->style()->position() != FixedPosition)
                continue;
            r->setChildNeedsLayout(true);
            hasFixedPositionedObjects = true;
        }
    }
    return hasFixedPositionedObjects;
}
#endif
...
void RenderBlock::paint(PaintInfo& paintInfo, const LayoutPoint& paintOffset)
{
    ...
    if (!isRoot()) {
#if OS(ANDROID)
        // Do not do overflowBox rejection for the accelerated scrolling case.
        if (!hasLayer() || !layer()->hasAcceleratedScrolling()) {
#endif
...
Node* RenderBlock::nodeForHitTest() {
    // If we are in the margins of block elements that are part of a continuation,
    // we're actually still inside the enclosing element that was
    // split.  Use the appropriate inner node.
...
bool RenderBlock::nodeAtPoint(const HitTestRequest& request, HitTestResult& result, const LayoutPoint& pointInContainer, const LayoutPoint& accumulatedOffset, HitTestAction hitTestAction)
{
    ...
    if ((hitTestAction == HitTestBlockBackground || hitTestAction == HitTestChildBlockBackground) && isPointInOverflowControl(result, pointInContainer, adjustedLocation)) {
        ...
        if (!result.addNodeToRectBasedTestResult(nodeForHitTest(), pointInContainer))
           return true;
    }
    ...
    if (hitTestAction == HitTestBlockBackground || hitTestAction == HitTestChildBlockBackground) {
        ...
        if (visibleToHitTesting() && boundsRect.intersects(result.rectForPoint(pointInContainer))) {
            ...
            if (!result.addNodeToRectBasedTestResult(nodeForHitTest(), pointInContainer, boundsRect))
                return true;
        }
    }
...
void RenderBlock::adjustComputedFontSizes(float size, float visibleWidth)
{
    ...
    // FIXME(johnme): Make sure these are both unscaled!!!!
    float fontScaleFactor = frame()->settings()->fontScaleFactor();
    if (visibleWidth / fontScaleFactor >= width()) {
        FB_SET_ATTR(this, "fb-status", "SKIP: visibleWidth >= width()");
        return;
    }
    ...
    FB_SET_ATTR(this, "fb-line-count", String::format("%d (enum=%d)", immediateLineCount(), lineCount));
    ASSERT(lineCount != NOT_SET);
    if (lineCount == NO_LINE) {
        FB_SET_ATTR(this, "fb-status", "SKIP: lineCount == NO_LINE");
        return;
    }

    // Don't boost block-level text that can't wrap (as it's likely to expand sideways
    // and break the page's layout). See also foundWrappableDescendant below.
    if (!style()->autoWrap())
        return;

    // Avoid boosting blocks with background images since we may cause text to spill
    // past the bottom of the image (particularly bad if the image is a sprite).
    if (style()->hasBackgroundImage()) {
        FB_SET_ATTR(this, "fb-status", "SKIP: style()->hasBackgroundImage()");
        return;
    }

    // Iterate through children to determine whether to skip font boosting.
    bool endOfInline = false;
    for (RenderObject* o = bidiFirstIncludingEmptyInlines(this); o; o = bidiNextIncludingEmptyInlines(this, o, &endOfInline)) {
        // Skip font boosting for textField or menuList.
        if ((o->isReplaced() || o->isFloating()) && !o->isPositioned() && (o->isTextField() || o->isMenuList())) {
            FB_SET_ATTR(this, "fb-status", "SKIP: (o->isReplaced() || o->isFloating() || o->isPositioned()) && (!o->isPositioned() && (o->isTextField() || o->isMenuList()))");
            return;
        }
    }

    // Traverse inline descendants and make sure at least one of them is wrappable.
    // As long as one of the descendent text nodes is wrappable, we will boost all
    // other descendent text nodes, and force them to wrap (in Document::setTextSize),
    // even if the author specified them to be unwrappable. This is because it looks
    // really bad to have different font sizes on the same line, so usually it's
    // better to wrap inline unwrappables than end up with inconsistent font sizes.
    // However when inline unwrappables are the only children of a block, including
    // e.g. a ul with every li set to unwrappable, since
    // includeNonBlockLevelForFontBoosting treats lis as inline, then we should play
    // it safe and not wrap or boost them (we can't boost things we can't wrap since
    // they're likely to expand sideways and break the layout).
    bool foundWrappableDescendant = false;
    for (RenderObject *descendant = traverseNext(this, includeNonBlockLevelForFontBoosting); descendant; descendant = descendant->traverseNext(this, includeNonBlockLevelForFontBoosting)) {
        if (isVisibleRenderText(descendant) && resizeTextPermitted(descendant) && descendant->style()->autoWrap()) {
            foundWrappableDescendant = true;
            break;
        }
    }
    if (!foundWrappableDescendant)
        return;

    // Calculate parameters.
    // FIXME(johnme): Make sure this is unscaled!!!!
    float actualWidth = m_widthForTextAutosizing != -1 ? static_cast<float>(m_widthForTextAutosizing) : static_cast<float>(width());
    float scale = visibleWidth / (actualWidth * fontScaleFactor);
    if (scale <= 0.0f || scale >= 1.0f) {
        FB_SET_ATTR(this, "fb-status", "SKIP: scale <= 0.0f || scale >= 1.0f");
        return;
    }

    bool blockHasClusters = false;

    // Traverse descendents and apply font boosting.
    for (RenderObject *descendent = traverseNext(this, includeNonBlockLevelForFontBoosting); descendent; descendent = descendent->traverseNext(this, includeNonBlockLevelForFontBoosting)) {
        if (isVisibleRenderText(descendent) && resizeTextPermitted(descendent)) {
            ...
            for (int depth = 0; depth < 30; depth++) {
                IntRect rect = current->absoluteBoundingBoxRectIgnoringTransforms();
                ancestors.append(String::format("%d: %04x %d %s (%d,%d; %d,%d; maxW %d); ",
                        depth,
                        reinterpret_cast<unsigned>(current) % 0x10000,
                        current->isRenderBlock(),
                        current->renderName(),
                        rect.x(),
                        rect.y(),
                        rect.width(),
                        rect.height(),
                        (current->isRenderBlock() ? toRenderBlock(current)->m_maxTextWidthAfterFontBoosting : -2)));
                current = current->parent();
                if (!current)
                    break;
            }
            FB_LOGF("%s", ancestors.utf8().data());
#endif

            OwnPtr<FontBoostingCluster> cluster = FontBoostingCluster::create(document(), this, text);
            // As FrameView::adjustViewSize hasn't been called since the most recent layout the values in contentsSize might be out of date or even be empty.
            float pageCSSWidth = (frame()->contentRenderer() ? frame()->contentRenderer()->documentRect().size().width() : frame()->view()->contentsSize().width()) / frame()->frameScaleFactor();
            if (cluster->numLinesOfText() <= 0 || cluster->boundingRect().maxX() < 0 || cluster->boundingRect().x() > pageCSSWidth) {
                FB_LOGF("skip hidden %08x  %d <= 0 || %d < 0 || %d > %f", FontBoostingCluster::cid(cluster.get()), cluster->numLinesOfText(), cluster->boundingRect().maxX(), cluster->boundingRect().x(), pageCSSWidth);
                continue; // Skip hidden elements (including e.g. text-indent:-9999px).
            }
            if (!blockHasClusters) {
                FB_LOGF("!shouldMergeInlines ???? %08x  !blockHasClusters", FontBoostingCluster::cid(cluster.get()));
                document()->addFontBoostingCluster(cluster.release()); // Start new cluster.
                blockHasClusters = true;
            } else {
                FontBoostingCluster* lastCluster = document()->lastFontBoostingCluster();
                if (!lastCluster->shouldMergeInlines(cluster.get())) {
                    document()->addFontBoostingCluster(cluster.release()); // Start new cluster.
                } else {
                    lastCluster->merge(cluster.release(), FontBoostingCluster::MergeInlines); // Join previous cluster.
                }
            }
            continue; // Too early to compute/set font sizes. We do this per-cluster, in Document::combineAndBoostFontBoostingClusters.
        }
    }
}

void RenderBlock::setMaxTextWidthAfterFontBoosting(int width)
{
    if ((m_maxTextWidthAfterFontBoosting != -1 && width != -1) || width == m_maxTextWidthAfterFontBoosting)
        return;

    m_maxTextWidthAfterFontBoosting = width;
    m_widthForTextAutosizing = width;
    FB_SET_ATTR(this, "fb-max-width", String::number(width));

    // Don't allow descendants (e.g. <li> descendants of a <ul>) to expand either.
    for (RenderObject* descendant = traverseNext(this, includeNonBlockLevelForFontBoosting); descendant; descendant = descendant->traverseNext(this, includeNonBlockLevelForFontBoosting))
        if (descendant->isRenderBlock())
            toRenderBlock(descendant)->setMaxTextWidthAfterFontBoosting(width);

    setNeedsLayout(true); // FIXME(johnme): See if there's any way we can avoid setNeedsLayout.
}
