...
// attribute setter methods
...
#if OS(ANDROID) && ENABLE(FONT_BOOSTING)
    void setBlendedFontSize(float specifiedSize, float computedSize);
#else
...
#if OS(ANDROID) && ENABLE(FONT_BOOSTING)
    bool setTextSizeAdjust(TextSizeAdjustment anAdjustment) {
    if (compareEqual(rareInheritedData->textSizeAdjust, anAdjustment))
        return false;
    ...
    uint32_t hashForFontBoosting() const;
    bool equalForTextAutosizing(const RenderStyle *other) const;
    bool equalForFontBoosting(const RenderStyle *other) const;
#endif
